...
async function removeRunner() {
  const runner = await getRunner(config.input.runnerLabel);
  const octokit = github.getOctokit(config.input.githubToken);

  // skip the runner removal process if the runner is not found
  if (!runner) {
    core.info(`GitHub self-hosted runner with label ${config.input.runnerLabel} is not found, so the removal is skipped`);
    return;
  }
